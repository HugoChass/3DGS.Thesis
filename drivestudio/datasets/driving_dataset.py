...
class DrivingDataset(SceneDataset):
    ...
    def get_lidar_samples(
        ...
        # NEW get semanics or if needed
        sampled_semantics = None
        if return_semantics:
            sampled_semantics = self.lidar_source.semantics[sampled_idx].to(device)
        
        return sampled_pts, sampled_color, sampled_time, sampled_semantics
    ...
    def get_init_objects(
        ...
        for fi in range(self.frame_num):
            ...
            for ins_id in range(self.instance_num):
                ...
                instance_dict[ins_id]["semantics"].append(valid_semantics)
        ...
        for ins_id in instance_dict:
            ...
            instance_dict[ins_id]["semantics"] = torch.cat(instance_dict[ins_id]["semantics"], dim=0)
            ...
            if instance_dict[ins_id]["num_pts"] > instance_max_pts:
                ...
                instance_dict[ins_id]["semantics"] = instance_dict[ins_id]["semantics"][sampled_idx]
    ...
    def filter_pts_in_boxes(
        ...
        if seed_semantics is not None:
            seed_semantics = seed_semantics[~inside_mask]
    ...
    def gaussian_semantic_thicken(self,
        ...
        num_classes: int,
        ignore_index: int,
        sigma: float = 5.0,           # blur in pixels; ~1–4 is typical
