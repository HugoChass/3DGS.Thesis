...
class NuScenesProcessor(object):
    ...
    def save_lidar(self, scene_data, scene_idx):
        ...
        while True:
            ...
            lidarseg_rec = self.nusc.get('lidarseg', lidar_token)
            label_relpath = lidarseg_rec['filename']
            ...
            if labels.shape[0] != pc.points.shape[1]:
                raise RuntimeError(
                    ...
                    f"{pc.points.shape[1]} points in {lidar_token}"
                )

            lidarseg_save_path = (
                f"{self.save_dir}/{str(scene_idx).zfill(3)}/lidarseg/{str(key_frame_idx).zfill(3)}.bin"
            )
    ...
    def create_folder(self):
        ...
        for i in id_list:
            ...
            if "lidar" in self.process_keys:
                ...
                os.makedirs(f"{self.save_dir}/{str(i).zfill(3)}/lidarseg", exist_ok=True)
            if "dynamic_masks" in self.process_keys:
