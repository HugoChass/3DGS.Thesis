...
class RigidNodes(VanillaGaussians):
    ...
    def create_from_pcd(self, instance_pts_dict: Dict[str, torch.Tensor], semantic=False) -> None:
        ...
        instances_semantics = torch.cat(instances_semantics, dim=0).to(self.device) # NEW
    ...
    def dup_gaussians(self, dup_mask: torch.Tensor) -> Tuple:
        ...
        return dup_means, dup_feature_dc, dup_feature_rest, dup_opacities, dup_scales, dup_quats, dup_ids, dup_semantics
    ...
    def get_instance_activated_gs_dict(self, ins_id: int) -> Dict[str, torch.Tensor]:
        ...
        gaussian_dict = {
            ...
            "semantics": self._semantics[pts_mask], # NEW
            "semantics_bool": self._semantics_bool[pts_mask],
        }
    ...
    def remove_instances(self, remove_id_list: List[int]) -> None:
        ...
        for ins_ids in remove_id_list:
            ...
            self._semantics = Parameter(self._semantics[mask])
            self._semantics_bool = self._semantics_bool[mask]
        ...
        for id in ids:
            if id not in gaussian_dict:
                instance_raw_dict = {
                    ...
                    "_semantics": self._semantics[self.point_ids[..., 0] == id], # NEW
                    "_semantics_bool": self._semantics_bool[self.point_ids[..., 0] == id], # NEW
    ...
    def replace_instances(self, replace_dict: Dict[int, int]) -> None:
        ...
        for ins_id, new_id in replace_dict.items():
            ...
            self._semantics = Parameter(torch.cat([self._semantics, new_gaussian["_semantics"]], dim=0)) # NEW
            self._semantics_bool = torch.cat([self._semantics_bool, new_gaussian["_semantics_bool"]], dim=0)
